...
export default function BottomNav({ active, onChange, theme }) {
  ...
  // On Home, the Seraphim theme's nav bar is baked into the reference
  // mockup's art (see /themes/seraphim/nav-bar.jpg) - reuse that image
  // pixel-for-pixel instead of the SVG icon set, with real (invisible)
  // buttons on top for taps and a gold glow marking whichever tab is
  // actually active. Every other tab keeps the normal SVG nav for now.
  if (theme === "seraphim" && active === "home") {
    return (
      <nav className="bottom-nav bottom-nav--seraphim">
        {TABS.map((tab) => (
          <button
            key={tab.id}
            type="button"
            className="bottom-nav__hit"
